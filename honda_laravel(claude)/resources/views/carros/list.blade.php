...
@section('title', 'Estoque de Carros')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between pb-5 border-b border-slate-800/80">
        ...
        <a href="{{ route('carros.create') }}" id="btn-novo-carro"
                ...
                <tbody class="divide-y divide-slate-800/60 text-sm text-slate-300">
                    @forelse($carros as $car)
                        <tr class="hover:bg-slate-900/30 transition-colors">
                            <td class="py-4 px-6 text-slate-500 font-mono text-xs">#{{ $car->id }}</td>
                            <td class="py-4 px-6 font-bold text-white">{{ $car->marca }}</td>
                            <td class="py-4 px-6 text-slate-300">{{ $car->modelo }}</td>
                            <td class="py-4 px-6">
                            ...
                            <td class="py-4 px-6 text-slate-400 font-medium">{{ $car->ano }}</td>
                            <td class="py-4 px-6 font-bold text-red-400">R$ {{ number_format($car->valor, 2, ',', '.') }}</td>
                            <td class="py-4 px-6 text-center">
                                <div class="flex items-center justify-center space-x-2">
                                    <a href="{{ route('carros.edit', $car->id) }}"
                                       id="btn-edit-carro-{{ $car->id }}"
                                       title="Editar"
                                    ...
                                    <form action="{{ route('carros.destroy', $car->id) }}" method="POST"
                                          onsubmit="return confirm('Deseja realmente excluir este carro?')" style="display:inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" id="btn-delete-carro-{{ $car->id }}" title="Excluir"
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="py-12 px-6 text-center text-slate-500">
                                Nenhum carro cadastrado. <a href="{{ route('carros.create') }}" class="text-red-400 hover:underline">Adicionar o primeiro.</a>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
