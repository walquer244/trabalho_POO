...
<div class="space-y-6">
    <div class="flex items-center justify-between pb-5 border-b border-slate-800/80">
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-white font-sans">Cadastrar Usuário</h1>
            <p class="text-slate-400 text-sm mt-1">Crie um novo login corporativo com privilégios de acesso</p>
        </div>
        ...
        <form action="create.php" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="nome" id="nome" required placeholder="Carlos Henrique"
                           value="{{ $nome ?? '' }}"
                ...
                <div>
                    ...
                    <input type="email" name="email" id="email" required placeholder="carlos@honda.com.br"
                           value="{{ $email ?? '' }}"
                           class="w-full bg-slate-950/60 border border-slate-800 hover:border-slate-700 focus:border-red-500 rounded-xl py-2.5 px-4 text-slate-200 placeholder-slate-600 focus:outline-none transition text-sm">
                </div>
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    <label for="senha" class="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">Senha</label>
                    <input type="password" name="senha" id="senha" required placeholder="Definir senha..."
                           class="w-full bg-slate-950/60 border border-slate-800 hover:border-slate-700 focus:border-red-500 rounded-xl py-2.5 px-4 text-slate-200 placeholder-slate-600 focus:outline-none transition text-sm">
                </div>
                <div>
                    ...
                    <select name="nivel_acesso" id="nivel_acesso"
                        ...
                        <option value="funcionario" {{ (isset($nivel_acesso) && $nivel_acesso === 'funcionario') ? 'selected' : '' }}>Funcionário</option>
                        <option value="admin" {{ (isset($nivel_acesso) && $nivel_acesso === 'admin') ? 'selected' : '' }}>Administrador</option>
                    </select>
                </div>
            </div>
