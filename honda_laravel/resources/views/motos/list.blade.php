...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between pb-5 border-b border-slate-800/80">
        ...
        <a href="create.php" id="btn-nova-moto"
                    ...
                    @if (count($motos) > 0)
                        @foreach ($motos as $moto)
                            <tr class="hover:bg-slate-900/30 transition-colors">
                                ...
                                <td class="py-4 px-6 font-bold text-amber-400">{{ format_currency((float)$moto->valor) }}</td>
                                <td class="py-4 px-6 text-center">
                                    <div class="flex items-center justify-center space-x-2">
                                        <a href="edit.php?id={{ $moto->id }}"
                                        ...
                                        <a href="delete.php?id={{ $moto->id }}"
                                           id="btn-delete-moto-{{ $moto->id }}"
                                           title="Excluir"
                                           onclick="return confirm('Deseja realmente excluir esta moto?')"
                    ...
                    @else
                        <tr>
                            <td colspan="7" class="py-12 px-6 text-center text-slate-500">
                                Nenhuma moto cadastrada. <a href="create.php" class="text-red-400 hover:underline">Adicionar a primeira.</a>
                            </td>
                        </tr>
                    @endif
                </tbody>
